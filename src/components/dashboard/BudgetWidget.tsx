...
export default function BudgetWidget({ totalBudget, currentExpenses }: BudgetWidgetProps) {
  ...
  return (
    <div className="px-6 mb-4">
      <div 
        className={`rounded-2xl p-1 shadow-[0_8px_20px_-10px_rgba(0,0,0,0.08)] border cursor-pointer hover:-translate-y-0.5 transition-all ${
          isOverBudget 
          ...
          <div className="flex flex-col min-w-0 flex-1 gap-1">
            <div className="flex items-center justify-between">
              <h3 className={`text-xs font-bold ${
                isOverBudget ? 'text-red-800' : isWarning ? 'text-yellow-800' : 'text-green-800'
              }`}>
                Presupuesto del mes
              </h3>
            ...
            <Progress 
              ...
              className={`h-1 ${
                isOverBudget ? 'bg-red-200' : isWarning ? 'bg-yellow-200' : 'bg-green-200'
              }`}
            />
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={`text-[10px] font-medium ${
                isOverBudget ? 'text-red-700' : isWarning ? 'text-yellow-700' : 'text-green-700'
              }`}>
