...
import { AssetType } from './AssetCard';
...
const AddAssetModal: React.FC<AddAssetModalProps> = ({ isOpen, onClose, onAdd }) => {
    ...
    const getTypeStyle = (t: AssetType) => {
        ...
        return `flex-1 py-2 rounded-xl text-xs font-bold transition-colors uppercase ${isSelected ? 'bg-[#5D4037] text-white shadow-md' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'
            }`;
    };
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button type="button" onClick={() => setType(AssetType.ETF)} className={getTypeStyle(AssetType.ETF)}>
                                ETF
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                        <label className="block text-[10px] font-bold text-gray-400 mb-1 ml-1 uppercase tracking-wider">Nombre (Opcional)</label>
                        <input
                            type="text"
                            value={name}
                    ...
                    <div>
                        <label className="block text-[10px] font-bold text-gray-400 mb-1 ml-1 uppercase tracking-wider">Fecha de Compra</label>
                        <input
                            required
                            type="date"
                            value={date}
