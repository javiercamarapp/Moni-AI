...
import { PlusIcon, MicIcon, WaveformIcon, SendIcon, SparklesIcon, CameraIcon, ImageIcon, ChartIcon, TelescopeIcon, GlobeIcon, BookIcon, UserIcon, PaperclipIcon, WalletIcon, ArrowLeftIcon } from '@/components/chat/Icons';
...
const FinancialChat = () => {
  ...
  const handleSend = useCallback(async (textOverride?: string) => {
    ...
    try {
      ...
      setMessages(prev => prev.map(msg => msg.id === aiMessageId ? {
        ...msg,
        text: "Tuve un pequeño problema de conexión. ¿Podrías repetirlo?",
        isThinking: false
      } : msg));
      return false;
    } finally {
  ...
  return <div className="page-standard min-h-screen flex flex-col items-center relative">
      <Waves lineColor="rgba(87, 83, 78, 0.125)" backgroundColor="transparent" waveSpeedX={0.02} waveSpeedY={0.01} waveAmpX={40} waveAmpY={20} friction={0.9} tension={0.01} maxCursorMove={120} xGap={12} yGap={36} className="fixed inset-0 z-0 pointer-events-none" />
      {/* Hidden File Inputs */}
      ...
      <main className="flex-1 relative w-full max-w-2xl mx-auto flex flex-col overflow-hidden pb-32">
        ...
        {appState === AppState.CHATTING && <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6 no-scrollbar">
          ...
          y: 0
                ...
                <div className={`max-w-[85%] px-5 py-3 rounded-2xl text-[15px] leading-relaxed shadow-sm ${msg.role === 'user' ? 'bg-coffee-900 text-coffee-50 rounded-br-sm border border-white/20' : 'bg-white text-coffee-900 border border-coffee-100/80 rounded-bl-sm'}`}>
                  {msg.role === 'model' && msg.isThinking ? <div className="flex gap-1.5 items-center h-6">
                    ...
                    </div> : <div className="whitespace-pre-wrap">{msg.text}</div>}
                </div>
              </motion.div>)}
      ...
      <div className="fixed bottom-20 left-0 right-0 z-20 px-4">
        <div className="w-full max-w-2xl mx-auto backdrop-blur-sm bg-coffee-50/90 p-4 rounded-3xl border border-white/20 shadow-[0px_15px_45px_-10px_rgba(0,0,0,0.25),0px_0px_10px_-2px_rgba(0,0,0,0.08)]">
          ...
          {appState === AppState.IDLE && <div className="w-full flex overflow-x-auto gap-2.5 pb-2 px-1 no-scrollbar snap-x snap-mandatory">
              {SUGGESTIONS.map((s, i) => <button key={i} onClick={() => handleAction(s.prompt)} className="snap-center shrink-0 w-auto min-w-[120px] max-w-[160px] p-3 rounded-2xl bg-coffee-600 text-coffee-50 hover:bg-coffee-700 transition-colors text-left flex flex-col gap-0.5 shadow-sm active:scale-95 duration-200">
          ...
          <div className="relative flex items-center gap-2 bg-coffee-100 p-1 sm:p-2 rounded-3xl shadow-sm border border-coffee-200/50 transition-all duration-200">
            ...
            <div className="flex items-center gap-1 shrink-0">
              {input.trim() || isListening ? <button onClick={input.trim() ? () => handleSend() : stopRecording} disabled={!input.trim() && !isListening} className={`w-8 h-8 flex items-center justify-center rounded-full transition-all duration-300 ${isListening ? 'bg-red-500 text-white animate-pulse' : 'bg-coffee-900 text-coffee-50 hover:bg-coffee-800'}`}>
                  {isListening ? <MicIcon /> : <SendIcon />}
                </button> : <>
                  <button onClick={handleMicClick} className="w-8 h-8 flex items-center justify-center rounded-full text-coffee-600 hover:bg-coffee-200/50 transition-colors">
                    <MicIcon />
                  </button>
                  <button onClick={() => setIsVoiceMode(true)} className="w-8 h-8 flex items-center justify-center rounded-full bg-coffee-900 text-coffee-50 hover:bg-coffee-800 transition-colors shadow-sm">
                    <WaveformIcon />
                  </button>
                </>}
            </div>
      ...
      <AnimatePresence>
        {isMenuOpen && <>
          ...
          opacity: 0
        }} onClick={() => setIsMenuOpen(false)} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" />
            <motion.div initial={{
          y: "100%"
        }} animate={{
          y: 0
          ...
          stiffness: 300
              ...
              <div className="grid grid-cols-4 gap-3 mb-8">
                <button onClick={() => cameraInputRef.current?.click()} className="aspect-square rounded-2xl bg-coffee-900/50 border border-coffee-800 flex items-center justify-center hover:bg-coffee-800 transition-colors">
                  <CameraIcon />
                </button>

                <button onClick={() => galleryInputRef.current?.click()} className="aspect-square rounded-2xl bg-coffee-900/50 border border-coffee-800 overflow-hidden relative group">
                ...
                <button onClick={() => galleryInputRef.current?.click()} className="aspect-square rounded-2xl bg-coffee-900/50 border border-coffee-800 flex items-center justify-center text-xs font-medium text-coffee-400 hover:text-white hover:bg-coffee-800 transition-colors">
                  +12
                </button>
              </div>
              ...
              <div className="space-y-1">
                <button onClick={() => fileInputRef.current?.click()} className="w-full p-4 hover:bg-coffee-900/50 rounded-2xl flex items-center gap-4 transition-colors group text-left">
                ...
                <button onClick={() => handleAction("Generar gráficas de finanzas personales")} className="w-full p-4 hover:bg-coffee-900/50 rounded-2xl flex items-center gap-4 transition-colors group text-left">
                  <div className="w-10 h-10 rounded-full bg-coffee-900 flex items-center justify-center text-coffee-400 group-hover:text-white transition-colors border border-coffee-800">
                    <ChartIcon />
                  </div>
                  <div>
                ...
                <button onClick={() => handleAction("Dame métricas personales de mis finanzas")} className="w-full p-4 hover:bg-coffee-900/50 rounded-2xl flex items-center gap-4 transition-colors group text-left">
                  <div className="w-10 h-10 rounded-full bg-coffee-900 flex items-center justify-center text-coffee-400 group-hover:text-white transition-colors border border-coffee-800">
                    <WalletIcon />
                  </div>
                  <div>
                ...
                <button onClick={() => handleAction("Recomendaciones de mejora financiera")} className="w-full p-4 hover:bg-coffee-900/50 rounded-2xl flex items-center gap-4 transition-colors group text-left">
                  <div className="w-10 h-10 rounded-full bg-coffee-900 flex items-center justify-center text-coffee-400 group-hover:text-white transition-colors border border-coffee-800">
                    <SparklesIcon />
                  </div>
                  <div>
      ...
      {/* Voice Mode Overlay */}
      <AnimatePresence>
        {isVoiceMode && <motion.div initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} exit={{
        opacity: 0
            ...
            <div className="absolute top-0 w-full flex flex-col items-center pt-12">
              ...
              <div className="px-6 py-2 bg-white border border-coffee-200 rounded-full shadow-sm">
                <span className="text-coffee-900 font-semibold animate-pulse">
                  {voiceStatus === 'listening' ? 'Escuchando...' : voiceStatus === 'processing' ? 'Procesando...' : 'Hablando...'}
                </span>
              </div>
            </div>
            ...
            <div className="relative w-64 h-64 flex items-center justify-center">
              {/* Outer Glow */}
              <motion.div animate={{
            scale: voiceStatus === 'listening' ? [1, 1.2, 1] : voiceStatus === 'processing' ? [1, 1.1, 1] : [1, 1.5, 1],
            opacity: voiceStatus === 'listening' ? 0.3 : voiceStatus === 'processing' ? 0.6 : 0.2
          }} transition={{
            ...
            repeat: Infinity
              ...
              {/* Core Sphere */}
              <motion.div animate={{
            scale: voiceStatus === 'listening' ? [1, 1.05, 1] : voiceStatus === 'processing' ? [0.9, 1.1, 0.9] : [1, 1.1, 1]
          }} transition={{
            ...
            <div className="absolute bottom-12 w-full flex justify-center gap-6 items-center">
              <button onClick={stopVoiceConversation} className="px-8 py-3 bg-coffee-200 hover:bg-coffee-300 text-coffee-900 rounded-full font-medium transition-colors shadow-sm">
                Cerrar
              </button>
            </div>
          </motion.div>}
