-- Add encryption validation and documentation for bank_connections

-- Add a comment to document that access_token must be encrypted
COMMENT ON COLUMN public.bank_connections.access_token IS 'MUST be encrypted using encrypt-bank-token function before storage. Never store plaintext tokens.';

-- Add a check to ensure tokens appear encrypted (basic validation)
-- Encrypted tokens from our function are base64 encoded and should be at least 32 chars
...
BEGIN
  -- Validate that token appears to be encrypted
